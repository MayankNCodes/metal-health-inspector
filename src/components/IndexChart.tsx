...
export const IndexChart: React.FC<IndexChartProps> = ({ results }) => {
  ...
  const pieData = [
    {
      name: "Heavy Pollution Index (HPI)",
      value: results.HPI,
      color: "#8884d8",
    },
    {
    ...
    {
      name: "Heavy Metal Pollution Index (HMPI)",
      value: results.HMPI,
      color: "#ffc658",
    },
    {
      name: "Pollution Load Index (PLI)",
      value: results.PLI * 20,
      color: "#ff7300",
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-card p-3 border rounded-lg shadow-lg">
          ...
          <p className="text-sm">
            Status:{" "}
            {payload[0].value < data.threshold * 0.25
              ? "Good"
              : payload[0].value < data.threshold * 0.5
              ? "Acceptable"
              : payload[0].value < data.threshold
              ? "Poor"
              : "Critical"}
          </p>
        </div>
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-card">
        <CardHeader>
          ...
          <CardDescription>
            Visual comparison of calculated pollution indices against threshold
            values
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <ResponsiveContainer width="100%" height={400}>
            <PieChart>
              <Pie
                ...
                labelLine={false}
                label={({ name, percent }) =>
                  `${Math.floor(percent.toFixed(2) * 100)}%`
                }
                outerRadius={80}
                fill="#8884d8"
                dataKey="value"
                activeShape={(props) => (
                  <Sector
                    {...props}
                    outerRadius={props.outerRadius + 10} // slice pops out smoothly
                    fill={props.fill}
                  />
                )}
              >
